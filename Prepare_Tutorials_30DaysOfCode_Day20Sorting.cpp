...
/**
Objective
    Today, we're discussing a simple sorting algorithm called Bubble Sort. Check
...
Example
    ...
    the last position. In the second round, the 3 is swapped at 2 of the
    3 comparisons. Finally, in the third round, no swaps are made so the
    iterations stop. The output is the following:
...
Sample Output 1
    ...
    Last Element: 3
Explanation 1
    The array a=[3,2,1] is not sorted, so we perform the following 3 swaps. Each
    line shows  after each single element is swapped.
    1. [3,2,1] -> [2,3,1]
    2.[2,3,1] -> [2,1,3]
    3.[2,1,3] -> [1,2,3]
