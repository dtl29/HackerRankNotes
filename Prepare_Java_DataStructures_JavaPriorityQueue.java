...
regular queue, but where additionally each element has a "priority" associated
with it. In a priority queue, an element with high priority is served before an
element with low priority. - Wikipedia
In this problem we will test your knowledge on Java Priority Queue.
There are a number of students in a school who wait to be served. Two types of
events, ENTER and SERVED, can take place which are described below.
ENTER: A student with some priority enters the queue to be served.
SERVED: The student with the highest priority is served (removed) from the
queue.
...
students based on the following criteria (priority criteria):
  ...
  3.Any students having the same CGPA and name will be served in ascending order
  of the id.
Create the following two classes:
  ...
  getStudents(List<String> events) to process all the given events and return
...
Explanation 0
  In this case, the number of events is 12. Let the name of the queue be Q.
    ...
    -Shafaet is added to Q. So, it contains (John, 3.75, 50), (Mark, 3.8, 24),
    and (Shafaet, 3.7, 35).
    -Mark is served as he has the highest CGPA. So, Q contains (John, 3.75, 50)
    and (Shafaet, 3.7, 35).
    ...
    (Shafaet, 3.7, 35).
    -Now, four more students are added to Q. So, it contains (Shafaet, 3.7, 35),
    (Ashley, 3.9, 42), (Maria, 3.6, 46), (Anik, 3.95, 49), and (Dan, 3.95, 50).
    -Anik is served because though both Anil and Dan have the highest CGPA but
...
class Priorities
{
    List<Student> getStudents(List<String> events)
    {
        ...
        for(int i = 0; i < events.size(); i++)
        {
            ...
            String[] et = temp.split(" ");
            String event = et[0];
            if(event.compareTo("ENTER") == 0)
            {
                String name = et[1];
                double gpa = Double.parseDouble(et[2]);
                int id = Integer.parseInt(et[3]);
                Student ns = new Student(id, name, gpa);
                pq.add(ns);
            }
            else
            {
