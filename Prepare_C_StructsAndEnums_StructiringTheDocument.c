...
([A-Z]) English characters. You will convert a raw text document into its
component paragraphs, sentences and words. To test your results, queries will
ask you to return a specific paragraph, sentence or word as described below.
...
The words in the sentence are separated by one space (" "). The last word does
...
The first sentence in the second paragraph could be represented as:
  ...
  first_sentence_in_second_paragraph.data = {"Learning", "pointers", "is",
  "more", "fun"};
The second sentence in the second paragraph could be represented as:
  struct sentence second_sentence_in_second_paragraph;
  second_sentence_in_second_paragraph.data = {"It", "is", "good", "to", "have",
  "pointers"};
The second paragraph could be represented as:
  struct paragraph second_paragraph;
  second_paragraph.data = {{"Learning", "pointers", "is", "more", "fun"}, {"It",
  "is", "good", "to", "have", "pointers"}};
Finally, the document could be represented as:
  struct document Doc;
  Doc.data = {{{"Learning", "C", "is", "fun"}}, {{"Learning", "pointers", "is",
  "more", "fun"}, {"It", "is", "good", "to", "have", "pointers"}}};
Alicia has sent a document to her friend Teodora as a string of characters, i.e.
represented by char* not struct document. Help her convert the document to
struct document form by completing the following functions:
    void initialise_document(char* text) to intialise the document. You have to
    intialise the global variable Doc of type struct document.
    struct paragraph kth_paragraph(int k) to return the kth paragraph in the
    document.
    struct sentence kth_sentence_in_mth_paragraph(int k ,int m ) to return the
    kth sentence in the mth paragraph.
    struct word kth_word_in_mth_sentence_of_nth_paragraph(int k, int m, to
...
Input Format
  ...
  3 k m n: This corresponds to calling the function
...
Explanation 0
  ...
  The third query returns the first word of the first sentence of the first
  paragraph.
*/
