...
Input Format
  ...
  The format for each test case is as follows:
    ...
    The next m lines contain an integer each, the elements of the second linked
    list.
Constraints
  1<=t<=10
  1<=n,m<=1000
...
SinglyLinkedListNode* mergeLists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    ...
    while(true)
    {
        if(head1 != nullptr && head2 != nullptr)
        {
            ...
            {
                nl->insert_node(head1->data);
                head1 = head1->next;
            }
            else
            {
                nl->insert_node(head2->data);
                head2 = head2->next;
            }
        }
    ...
    SinglyLinkedListNode* h = nl->head;
    while(true){
        cout << h->data << " ";
        if(h->next == nullptr)
        {
