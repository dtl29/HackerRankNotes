...
computational server that performs complex calculations.
...
Input Format
	...
	Each of the T subsequent lines describes a test case as 2 space-separated integers, A 
	and B, respectively.
...
Output Format
	...
	For each test case, print a single line containing whichever message described in the 
	Problem Statement above is appropriate. After all messages have been printed, the locked
	stub code in your editor prints the server load.
...
Explanation
	...
	-8 is negative, hence 'Exception: A is negative' is thrown. Since the second input is 
	too large, 'not enough memory' is displayed. 2 is the server load.
**/
...
try 
        {
            Server S;
            //don't throw the exception will be made eitherway 
            //throw S.compute(A,B);
