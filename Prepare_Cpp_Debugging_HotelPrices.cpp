...
The prices of a standard hotel room and hotel apartment are given as:
    ...
    Hotel Apartment: The price of a standard room with the same
     number bedrooms and bathrooms plus 100.
...
Input Format
    ...
    In the first line, there is a single integer n denoting the
    number of rooms booked for today.
    After that n lines follow. Each of these lines begins with a
    room_type which is either standard or apartment, and is followed
    by the number of bedrooms and the number of bathrooms in this
    room.
Constraints
    1<=n<=100
...
Output Format
    ...
    It calculates the total profit by iterating through the vector of
    all rooms read from the input.
Sample Input 0
    2
    standard 3 1
    apartment 1 1
Sample Output 0
    500
Explanation 0
    In the sample we have one standard room with 3 bedrooms and 1
    bathroom, and one apartment with one 1 bedrooms and 1 bathroom.
    The price for the room is 3*50+100=250. The price for the
    apartment is 50+100+100+250=250. Thus the hotel profit is
    250+250=500 as the sum of prices of both rooms.
**/
...
public:
    ...
    //needed to be a virtual function for the pointer to point to the derived class instead of the base class funciton
